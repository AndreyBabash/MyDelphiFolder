...
begin
  MyForm.DevicesMarkerArr[MyForm.incvalue] := MyForm.MyMap.AddMarker(MyForm.MyMarkerDescr); // Присваиваем маркеру описание и добавляем на карту
end;
...
begin
  ...
  // Проверяем, есть ли маркер на карте
      ...
      begin
        MyForm.DevicesMarkerArr[i].Remove;  // Удаляем маркер
        MyForm.DevicesMarkerArr[i]:=nil;    // присваиваем маркеру "пустоту"
      end;
  end;
end;
 ...
 { for i:=0 to MyForm.JSONMyArr.Size-1 do
  ...
  if MyForm.DevicesMarkerArr[i]=nil then
    begin
        ...
        MyForm.MyMarkerDescr := TMapMarkerDescriptor.Create(MarkerLocation, MyForm.TransportArr[i].route_long_name);  // Создаем описание маркера

        MyForm.IconIdentification(MyForm.TransportArr[i].route_short_name,MyForm.ImageList1,MyForm.MyMarkerDescr);

        MyForm.MyMarkerDescr.Snippet:='Скорость: '+MyForm.TransportArr[i].speed.ToString+' км/ч'+#13#10
        +'Время: '+MyForm.TransportArr[i].time+#13#10+'Номер: '+MyForm.TransportArr[i].gov_number+#13#10+
        'Маршрут: '+MyForm.TransportArr[i].route_short_name+#13#10+'Тип маршрута: '+MyForm.TransportArr[i].route_type+
        #13#10+'imei: '+MyForm.TransportArr[i].imei.ToString;
        MyForm.MyMarkerDescr.Draggable := False;
        MyForm.incvalue:=i;
        Synchronize(AddMarkerToMap);
        //MyForm.DevicesMarkerArr[i] := MyForm.MyMap.AddMarker(MyMarkerDescr); // Присваиваем маркеру описание и добавляем на карту
    end;
     MyForm.incvalue:=i;
     Synchronize(ShowProgress);
